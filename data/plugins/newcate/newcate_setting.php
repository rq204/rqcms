...
function newcate_html_view()
{
	global $DB,$hostid;
	$arr=$DB->fetch_first('select * from '.DB_PREFIX."plugin where `hostid`=$hostid and `file`='newcate'");
...
<input type="hidden" value="stat" name="plugin">
  ...
  <tr class="tablecell">
	<td width="20%"><b>统计代码:</b><td><textarea id="newcate_code" class="formarea" type="text" name="newcate_code" style="width:400px;height:80px;">$code</textarea></td>
  </tr>
    <tr class="tablecell">
...
addAction('admin_plugin_setting_view','newcate_html_view');

//保存统计代码
function newcate_code_save()
{
	global $DB,$hostid;
	$code=$_POST['newcate_code'];
...
addAction('admin_plugin_setting_save','newcate_code_save');
