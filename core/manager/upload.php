...
$dirType=$host['attach_save_dir'];//0,全部存到一个目录下 1:按分类存放 2:按月存入目录 3:按扩展名存目录

$cid=isset($_GET['cid'])?intval($_GET['cid']):'';
if($dirType=='1'&&!$cid) exit("{'err':'".jsonString('您设置了按分类上传文件名，请先选择分类')."','msg':''}");
...
if(!is_dir($attachDir))
{
	exit("{'err':'".jsonString('目录'.$attachDir.'不存在，请手工在网站根目录建立')."','msg':''}");
}
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			switch($dirType)
			{
				...
				case 1: $attachSubDir = $cid; break;
				case 2: $attachSubDir = 'month_'.date('ym'); break;
				case 3: $attachSubDir = $extension; break;
				//全部存到一个目录下 1:按分类存放 2:按月存入目录 3:按扩展名存目录
			}
			if($attachSubDir) $attachSubDir='/'.$attachSubDir;
...
exit("{'err':'".jsonString($err)."','msg':".$msg."}");
