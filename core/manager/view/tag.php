    ...
    <tr>
	  ...
	  <div class="tableborder">
        ...
        <div class="leftmenubody">
          <div class="leftmenuitem">&#8226; <a href="{$admin_url}?file=tag&action=taglist">标签管理</a></div>
		  <div class="leftmenuitem">&#8226; <a href="{$admin_url}?file=tag&action=tagrebuilt&tagindex=0">重建标签</a></div>
        </div>
	  </td>
      <td valign="top" style="width:20px;"></td>
      <td valign="top">
	  <form action="{$admin_url}?file=tag" method="POST"><table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
...
foreach($tagdb as $key => $tag){print <<<EOT
    <tr class="tablecell">
      ...
      <td>$tag[usenum]</td>
      <td><a href="{$admin_url}?file=tag&action=modtag&tag=$tag[url]">修改</a></td>
...
foreach($redirectdb as $key => $rdb){print <<<EOT
    <tr class="tablecell">
      ...
      <td>$rdb[new]</td>
	  <td>$rdb[status]</td>
      <td><a href="{$admin_url}?file=tag&action=modredirect&rid=$rdb[rid]">修改</a></td>
